...
public final class AlignmentInterval {
    ...
    static final void checkValidArgument(final Cigar cigar, final SimpleInterval referenceSpan,
        ...
        final int softClippedBases = SvCigarUtils.checkCigarAndConvertTerminalInsertionToSoftClip(cigar).getCigarElements().stream().filter(ce -> ce.getOperator().equals(CigarOperator.S)).mapToInt(CigarElement::getLength).sum();
